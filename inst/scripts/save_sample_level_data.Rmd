...
combine_normalized_data = function(gsutil_path, 
  ...
  for(f in file_list){
    ...
    if(colname_type == "pid"){
      ...
      curr_norm = fread(f, sep='\t', header=T, skip = 0) 
      if(colnames(curr_norm)[1]!='viallabel'){
        ...
        # remove pid, bid
        assert(curr_norm[1,viallabel] == "pid")
        assert(curr_norm[2,viallabel] == "bid")
    ...
    if(rename_features){
      ...
      setnames(curr_norm, colname_type, "feature_ID")
    }
...
```{r methyl raw counts}
# raw counts for DEA
files = system("gsutil ls gs://motrpac-data-freeze-pass/pass1b-06/v1.1/results/epigenomics/*/epigen-rrbs/motrpac_pass1b-06_*_epigen-rrbs_bismark-cov.RData", intern=T)
# system(sprintf("gsutil -m cp %s %s/METHYL",
#                paste(files, collapse=" "),
#                datadir))
for(f in basename(files)){
  # load it
  load(sprintf("%s/METHYL/%s", datadir, f))
  
  # get tissue abbreviation
  tissue_code = unname(unlist(strsplit(f, '_')))[3]
  tissue = MotrpacBicQC::tissue_abbr[[tissue_code]]
  
  # extract counts 
  counts = as.data.frame(rrbs_data$counts)
  
  # save to list 
  
  # # 6611282 rows --> 222M
  # # max 50MB per file for GitHub
  # # ~1M rows per file 
  # nchunks = ceiling(nrow(counts)/1e6)
  # print(nchunks)
  # nrow_per_chunk = ceiling(nrow(counts)/nchunks)
  # for (i in 1:nchunks){
  #   start = nrow_per_chunk*(i-1)+1
  #   end = min(nrow_per_chunk*i, nrow(counts))
  #   chunki = counts[start:end,]
  #   
  #   new_name = sprintf("%s_%s_RAW_COUNTS_P%s","METHYL",gsub("-","",tissue), i)
  #   writeLines(new_name)
  #   assign(new_name, chunki)
  #   print(dim(get(new_name)))
  #   print(head(get(new_name)))
  #   # Save .rda 
  #   if(!file.exists(sprintf("/oak/stanford/groups/smontgom/nicolerg/src/MOTRPAC/MotrpacRatTraining6moData/data/%s.rda",new_name))){
  #     do.call("use_data", list(as.name(new_name), overwrite = FALSE))
  #   }
  # }
}
