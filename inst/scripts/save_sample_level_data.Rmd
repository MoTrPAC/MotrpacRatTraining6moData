...
datadir = '/oak/stanford/groups/smontgom/shared/motrpac/internal_releases/motrpac-data-freeze-pass/pass1b-06/v1.0/normalized-sample-level'
...
```{r fx}
combine_normalized_data = function(gsutil_path, pattern, ome_abbreviation, datadir, gsutil_bin = '~/google-cloud-sdk/bin/gsutil', pheno = NULL, features_to_keep = NULL, tissue_index=3, skip=1, file_list=NULL, viallabel=F, add_filename_as_column=F){
  norm_list = list()
...
# rna
norm_data_list[["TRNSCRPT"]] = combine_normalized_data("gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/transcriptomics/transcript-rna-seq/normalized-data", "normalized-log-cpm", "TRNSCRPT", datadir, pheno=pheno)

# atac
atac = combine_normalized_data("gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/epigenomics/epigen-atac-seq/normalized-data", "quant-norm", "ATAC", datadir, pheno=pheno, features_to_keep = rownames(zs_smoothed)[grepl("ATAC", rownames(zs_smoothed))], tissue_index = 4)
length(rownames(zs_smoothed)[grepl("ATAC", rownames(zs_smoothed))])
dim(atac)
norm_data_list[['ATAC']] = atac

# rrbs
rrbs = combine_normalized_data("gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/epigenomics/epigen-rrbs/normalized-data", "normalized-log-M-window.txt", "METHYL", datadir, pheno=pheno, features_to_keep = rownames(zs_smoothed)[grepl("METHYL", rownames(zs_smoothed))])
length(rownames(zs_smoothed)[grepl("METHYL", rownames(zs_smoothed))])
dim(rrbs)
norm_data_list[['METHYL']] = rrbs

# prot-pr
prot = combine_normalized_data("gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/proteomics-untargeted/prot-pr/normalized-data", "med-mad-normalized-logratio.txt", "PROT", datadir, pheno=pheno)
norm_data_list[['PROT']] = prot

# prot-ac
ac = combine_normalized_data("gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/proteomics-untargeted/prot-ac/normalized-data", "med-mad-normalized-logratio.txt", "ACETYL", datadir, pheno=pheno)
norm_data_list[['ACETYL']] = ac

# prot-ub
ub = combine_normalized_data("gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/proteomics-untargeted/prot-ub/normalized-data", "med-mad-normalized-protein-corrected-logratio.txt", "UBIQ", datadir, pheno=pheno)
norm_data_list[['UBIQ']] = ub  

# prot-ph
ph = combine_normalized_data("gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/proteomics-untargeted/prot-ph/normalized-data", "med-mad-normalized-logratio.txt", "PHOSPHO", datadir, pheno=pheno)
norm_data_list[['PHOSPHO']] = ph
```

```{r immuno}
immuno = combine_normalized_data("gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/proteomics-targeted/immunoassay-luminex/normalized-data", "merged_mfi-log2-filt-imputed.txt", "IMMUNO", datadir, skip=0, tissue_index = 4)

plasma = fread("/oak/stanford/groups/smontgom/shared/motrpac/internal_releases/motrpac-data-freeze-pass/pass1b-06/v1.0/normalized-sample-level/IMMUNO/motrpac_20211019_pass1b-06_t31-plasma_immunoassay_merged_mfi-log2-filt-imputed.txt", header=T)
...
```{r deal with metab}
## metab
decision_table = dl_read_gcp("gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/metabolomics-named-merged/stats-tests/pass1b-06_sample-ctr-decision-table-kw-summary.txt", sep="\t")

# only do this once
# # first, download all relevant data 
# system(sprintf("mkdir -p %s/%s", datadir, "METAB"))
# system(sprintf("gsutil cp gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/metabolomics-named-merged/normalized-data/*imputed_named-convert2na-log2-featurectr-featurefilt-knn* %s/METAB", datadir))
# system(sprintf("gsutil cp gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/metabolomics-named-merged/normalized-data/*named-convert2na-log2.txt %s/METAB", datadir))
# system(sprintf("gsutil cp gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/metabolomics-named-merged/normalized-data/*named-convert2na-log2-featurectr-featurefilt.txt %s/METAB", datadir))
# system(sprintf("gsutil cp gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/metabolomics-named-merged/normalized-data/*named-featurefilt-knn-samplefilt-log2-featurestd.txt %s/METAB", datadir))
# system(sprintf("gsutil cp gs://motrpac-data-freeze-pass/pass1b-06/v1.0/analysis/metabolomics-named-merged/normalized-data/*named-featurefilt-knn-samplefilt-log2-featurestd-samplectr.txt %s/METAB", datadir))
...
# metab = combine_normalized_data('', '', 'METAB', datadir, gsutil_bin = '~/google-cloud-sdk/bin/gsutil', pheno=pheno, 
#                                 file_list=c(untargeted_file_list, targeted_file_list), tissue_index = 2, skip=0, viallabel=T,
#                                 add_filename_as_column = T)
# # add dataset 
# metab[,dataset := unname(unlist(sapply(origin_file, function(x){
#   splits = unname(unlist(strsplit(x,'_')))
#   dataset = paste0(splits[3:5], collapse='-')
#   return(dataset)
# })))]
# # reorder columns 
# metab = metab[,c('feature','origin_file','dataset',colnames(metab)[!colnames(metab)%in%c('feature','origin_file','dataset')]), with=F]
# write.table(metab, file = sprintf("%s/metabolomics-all-sample-level-data.tsv", datadir), sep='\t', col.names=T, row.names=F, quote=F)
```
...
norm_data = rbindlist(norm_data_list, fill = T)
...
saveRDS(norm_data_df, file=sprintf("%s/signif-epigen-features_all-other-features_sample-level-data_20220220.RDS", datadir))
write.table(norm_data_df, file=sprintf("%s/signif-epigen-features_all-other-features_sample-level-data_20220220.tsv", datadir), sep="\t", col.names=T, row.names=F, quote=F)
system(sprintf("gsutil -m cp -n %s/signif-epigen-features_all-other-features_sample-level-data_20220220* gs://mawg-data/pass1b-06/merged/freeze-sample-level-data/",datadir))

# filter down to differential features
norm_data_diff = norm_data[feature %in% differential_features]
dim(norm_data_diff)
length(unique(norm_data_diff[,feature]))
length(differential_features)
table(differential_features %in% norm_data_diff[,feature]) # WOO!!!

# look at duplicate feature names
norm_data_diff[feature %in% feature[duplicated(feature)]]

# convert to data.frame for others
diff_sample_level = as.data.frame(norm_data_diff)

# save
saveRDS(diff_sample_level, file=sprintf("%s/dea-0.05fdr-sample-level-data_20211212.RDS", datadir))
write.table(diff_sample_level, file=sprintf("%s/dea-0.05fdr-sample-level-data_20211212.tsv", datadir), sep="\t", col.names=T, row.names=F, quote=F)
system(sprintf("gsutil -m cp -n %s/dea-0.05fdr-sample-level-data_20211212* gs://mawg-data/pass1b-06/merged/freeze-sample-level-data/",datadir))
# gs://mawg-data/pass1b-06/merged/dea-0.05fdr-sample-level-data_20211212.RDS
# gs://mawg-data/pass1b-06/merged/freeze-sample-level-data/dea-0.05fdr-sample-level-data_20211212.RDS
```

```{r}
diff_sample_level = readRDS("/oak/stanford/groups/smontgom/shared/motrpac/internal_releases/motrpac-data-freeze-pass/pass1b-06/v1.0/normalized-sample-level/dea-0.05fdr-sample-level-data_20211212.RDS")
head(diff_sample_level)
```
