...
author: "Pierre M Jean Beltran, Nicole Gay"
date: "7/16/2022"
...
for(assay_code in names(METAB_NORM_DATA_NESTED)){
  for(tissue in names(METAB_NORM_DATA_NESTED[[assay_code]])){
    ...
    # this accounts for redundant features but NOT feature_IDs from metaregression, which is addressed below
    ...
    ## since moved to METAB_FEATURE_ID_MAP
...
METAB_FEATURE_ID_MAP[metabolite_name == "TG(36:1)>TG(4:0_16:0_16:1)_and_TG(2:0_16:0_18:1)_feature4", 
                     metabolite_refmet := refmet] 
...
### Fix `METAB_NORM_DATA_FLAT$feature`
```{r}
load("../../data/METAB_NORM_DATA_FLAT.rda")
load("../../data/METAB_FEATURE_ID_MAP.rda")
load("../../data/REPEATED_FEATURES.rda")
load("../../data/TRAINING_REGULATED_FEATURES.rda")

# 'feature' should only exist for training-differential features 
metab_diff = unique(TRAINING_REGULATED_FEATURES$feature[TRAINING_REGULATED_FEATURES$assay=="METAB"])
table(na.omit(METAB_NORM_DATA_FLAT$feature) %in% metab_diff)
table(metab_diff %in% METAB_NORM_DATA_FLAT$feature)

# so there are 289 differential metabolites not named in METAB_NORM_DATA_FLAT
missing = metab_diff[!metab_diff %in% METAB_NORM_DATA_FLAT$feature]
missing
# these look like RefMet IDs 
missing_featureID = unique(sapply(missing, function(x) unname(unlist(strsplit(x, ";")))[3]))
table(missing_featureID %in% METAB_FEATURE_ID_MAP$metabolite_refmet) # all but "meta-reg:GMP"

# for each of the missing features, find corresponding refmet
# replace feature
map = data.table(METAB_FEATURE_ID_MAP)
map[,wrong_feature := sprintf("METAB;%s;%s", tissue, metabolite_name)]
METAB_NORM_DATA_FLAT = data.table(METAB_NORM_DATA_FLAT)
for (m in missing){
  sub = map[feature==m]
  # for each metabolite_name, change "feature" in sample-level data 
  for(wrong_name in unique(sub[,wrong_feature])){
    .tissue = unname(unlist(strsplit(wrong_name, ";")))[2]
    .feature_ID = unname(unlist(strsplit(wrong_name, ";")))[3]
    METAB_NORM_DATA_FLAT[tissue == .tissue & feature_ID == .feature_ID, feature := m]
  }
}
...
table(na.omit(METAB_NORM_DATA_FLAT[,feature]) %in% metab_diff)
table(metab_diff %in% METAB_NORM_DATA_FLAT[,feature])
f = metab_diff[!metab_diff %in% METAB_NORM_DATA_FLAT[,feature]]
map[feature==f]
METAB_NORM_DATA_FLAT[feature_ID == "sn-glycero-3-phosphocholine" & tissue == "WAT-SC" & dataset == "metab-u-lrppos", feature := f]
...
# fixed!
# save and recompress
METAB_NORM_DATA_FLAT = as.data.frame(METAB_NORM_DATA_FLAT)
use_data(METAB_NORM_DATA_FLAT, overwrite = TRUE)
tools::resaveRdaFiles(paths = '../../data/METAB_NORM_DATA_FLAT.rda')
```

So now we've fixed the sample-level data, but what about the METAB_DA tables?
```{r check metab da}
# load all DEA tables 
obj_list = list_available_data(package="MotrpacRatTraining6moData")
metab_dea = obj_list[grepl("^METAB_",obj_list) & grepl("_DA$",obj_list)]
...
table(na.omit(dea[,feature]) %in% metab_diff)
table(metab_diff %in% na.omit(dea[,feature]))
missing = metab_diff[!metab_diff %in% na.omit(dea[,feature])]

# same problem... 
for(.tissue in unique(dea[,tissue])){
  # load table
  table_name = sprintf("METAB_%s_DA", gsub("-","",.tissue))
  data = fetch_object(table_name)
  data = data.table(data)
  
  # get missing metabolites for this tissue
  curr_missing = missing[grepl(sprintf("METAB;%s;",.tissue), missing)]
  if(length(curr_missing) == 0) next
  for (m in curr_missing){
    sub = map[feature==m]
    if(nrow(sub) == 0){
      print(m)
    }
    ...
    for(wrong_name in unique(sub[,wrong_feature])){
      .tissue = unname(unlist(strsplit(wrong_name, ";")))[2]
      .feature_ID = unname(unlist(strsplit(wrong_name, ";")))[3]
      data[tissue == .tissue & feature_ID == .feature_ID, feature := m]
    }
  }
  if(tissue == "WAT-SC"){
    data[feature_ID == "sn-glycero-3-phosphocholine" & tissue == "WAT-SC" & dataset == "metab-u-lrppos", 
         feature := "METAB;WAT-SC;sn-glycero-3-phosphocholine"]
    data[feature_ID == "sn-glycero-3-phosphocholine" & tissue == "WAT-SC" & dataset == "metab-u-rppos", 
         feature := "METAB;WAT-SC;sn-Glycero-3-phosphocholine"]
  }
  # make sure all metabolites have been covered 
  stopifnot(all(curr_missing %in% data[,feature]))
  
  # save again as object name 
  data = as.data.frame(data)
  # rename
  assign(table_name, data)
  do.call("use_data", list(as.name(table_name), overwrite = TRUE))
  # re-compress
  tools::resaveRdaFiles(paths = sprintf('../../data/%s.rda', table_name))
}
...
Lastly, what about the METAB_DA_METAREG tables?
```{r check metab da metareg}
# load all DEA tables 
obj_list = list_available_data(package="MotrpacRatTraining6moData")
metab_dea = obj_list[grepl("^METAB_",obj_list) & grepl("_DA_METAREG$",obj_list)]
dea_list = list()
for(dea in metab_dea){
  load(sprintf("../../data/%s.rda", dea))
  dea_list[[dea]] = as.data.table(get(dea))
...
# at least we don't have to change the metareg tables 
```
